...
+ (HYBRequestOperation *)uploadWithFile:(NSData *)data
                                     url:(NSString *)url
                                filename:(NSString *)filename
...
//    [manager.requestSerializer setValue:@"multipart/form-data" forHTTPHeaderField:@"Content-Type"];
    AFHTTPRequestOperation *op = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        // 上传图片，以文件流的格式
        [formData appendPartWithFileData:data name:name fileName:imageFileName mimeType:@"file"];
    } success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [self successResponse:responseObject callback:success];
...
+ (HYBRequestOperation *)uploadWithFiles:(NSArray *)dics
                                    url:(NSString *)url
                             parameters:(NSDictionary*)parameters
                                   name:(NSString *)name
                               progress:(HYBUploadProgress)progress
    ...
    AFHTTPRequestOperation *op = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (NSDictionary *dic in dics) {
            ...
            // 上传图片，以文件流的格式
            [formData appendPartWithFileData:data name:name fileName:imageName mimeType:@"file"];
...
+ (HYBRequestOperation *)uploadWithFilePathes:(NSArray *)dics
                                     url:(NSString *)url
                              parameters:(NSDictionary*)parameters
                                    name:(NSString *)name
                                progress:(HYBUploadProgress)progress
